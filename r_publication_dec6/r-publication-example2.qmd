...
When I made these changes, the author changed to my name, the theme changed so the page is now dark blue and the text is light gray, there's now a table of contents in the upper right corner and there is a dropdown in the codeblock section that displays the code.
...
I expected the results to flip, so we would see the output but no gray box for the first one and the gray box but no output for the second one, but nothing changed.
