...
If you inner_join these tables, you won't get accurate results because "name" in the first table includes the county and the state, but "name" in the second table only has the county name. They won't be recognized as being the same, so you would have to clean the data to remove the state name. Then, they will be recognized as the same and the tables can be joined.
